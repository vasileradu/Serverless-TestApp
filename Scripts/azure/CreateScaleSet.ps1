...
az vmss create --image "vmi-$appName" --name "vmss$appName" --resource-group TestApp --vm-sku "$size" --admin-username "TestAppAdmin" --admin-password "Pa55word!Pa55word!" --nsg TestApp-ngs --instance-count 1 --app-gateway "AG_$appName" --subnet "TestApp-subnet" --vnet-name "TestApp-vnet" --upgrade-policy-mode "Automatic" --tags "persistance=onetime" --public-ip-per-vm --backend-port 5000
...
az monitor autoscale rule create -g TestApp --autoscale-name "autoscale$appName" --condition "Percentage CPU < 20 avg 10m" --scale in 1
